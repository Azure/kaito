...
func TestUpdateWorkspaceStatus(t *testing.T) {
	t.Run("Should update workspace status successfully", func(t *testing.T) {
		...
		err := updateObjStatus(ctx, reconciler.Client, workspace, &client.ObjectKey{Name: workspace.Name, Namespace: workspace.Namespace}, &condition, workerNodes)
		assert.Nil(t, err)
	})
...
func TestUpdateStatusConditionIfNotMatch(t *testing.T) {
	t.Run("Should not update when condition matches", func(t *testing.T) {
		...
		err := updateStatusConditionIfNotMatch(ctx, workspace, workspace, reconciler.Client, &client.ObjectKey{Name: workspace.Name, Namespace: workspace.Namespace},
			workspace.Status.Conditions, conditionType, conditionStatus, conditionReason, conditionMessage)
		assert.Nil(t, err)
	})
