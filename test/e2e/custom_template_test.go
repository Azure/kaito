...
var _ = Describe("Inference Custom template", func() {
	It("should create a workspace for inference with custom template successfully", func() {
		workspaceObj := &kaitov1alpha1.Workspace{}
		machineObj := v1alpha5.Machine{}
		labelSelector := &metav1.LabelSelector{
			MatchLabels: map[string]string{"kaito-workspace": "custom-template-e2e-test"},
		}

		By("Creating a workspace CR with custom template", func() {
			workspaceObj = utils.GenerateWorkspaceManifest(fmt.Sprintf("custom-%d", rand.Intn(1000)), namespaceName,
				"", 1, "Standard_NC12s_v3",
				labelSelector, nil, "", utils.InferenceModeCustomTemplate, nil,
				utils.GeneratePodTemplate(fmt.Sprintf("custom-template-%d", rand.Intn(1000)),
					namespaceName, "nginx:latest", labelSelector.MatchLabels))

			// Create workspace
			Eventually(func() error {
				return TestingCluster.KubeClient.Create(ctx, workspaceObj, &client.CreateOptions{})
			}, 120*time.Minute, utils.PollInterval).
